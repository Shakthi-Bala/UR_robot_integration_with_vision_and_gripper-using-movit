    ...
    def execute_cb(self, goal):
        ...
        if Gripper_flag:
            try:
                ...
                after_end_pose = Pose()
                after_end_pose.position.x = end_pose.x
                after_end_pose.position.y = end_pose.y
                after_end_pose.position.z = end_pose.z + 0.35
                after_end_pose.orientation.x = 0.0
                after_end_pose.orientation.y = 1.0  
                after_end_pose.orientation.z = 0.0
                after_end_pose.orientation.w = 0.0 

                pose_stamped = PoseStamped()
                pose_stamped.pose = after_end_pose
                pose_stamped.header.frame_id = "base_link"  # Set the appropriate frame ID
                self.arm_move_group_.set_pose_target(pose_stamped)
